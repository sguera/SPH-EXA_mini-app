...
# broadwell:
# #{{{!
# @rfm.parameterized_test(*[[mpitask, cpubind, prg]
#                          for mpitask in [36, 72, 144, 216, 288]
#                          for cpubind in ['none']
#                          for prg in [('dom', 'PrgEnv-intel', 'intel/19.0.1.144')]])
#                         # for mpitask in [1, 2, 3, 4, 6, 9, 12, 18, 36]
# #                         for mpitask in [1, 2, 18, 36]
# #                         for cpubind in ['', 'none', 'sockets', 'cores',
# #                                         'threads', 'ldoms', 'boards']
# #                         for prg in [
...
@rfm.parameterized_test(*[[mpitask, cpubind, prg]
                         for mpitask in [12, 24, 48, 96, 192, 384, 768, 1536]
                         for cpubind in ['none']
                         for prg in [('daint', 'PrgEnv-intel', 'intel/18.0.2.199')]])
class SphExaMiniAppSquarepatchHaswellTest(SphExaMiniAppSquarepatchBaseTest):
    def __init__(self, mpitask, cpubind, prg):
        super().__init__()
        ompthread = '{:03d}'.format(int(12/mpitask))
        ...
        self.name = 'sphexa_%s_' % self.testname + \
