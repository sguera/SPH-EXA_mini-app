...
#pragma omp task
        {
            ...
            // CHECK_CUDA_ERR(cudaMemcpyAsync(d_clist, t.clist.data(), size_n_int, cudaMemcpyHostToDevice));
            // CHECK_CUDA_ERR(cudaMemcpyAsync(d_neighbors, t.neighbors.data(), size_nNeighbors, cudaMemcpyHostToDevice));
            // CHECK_CUDA_ERR(cudaMemcpyAsync(d_neighborsCount, t.neighborsCount.data(), size_n_int, cudaMemcpyHostToDevice));
            ...
            // kernels::density<<<blocksPerGrid, threadsPerBlock, 0, streams[i]>>>(n, d.sincIndex, d.K, t.ngmax, d_bbox, d_clist,
            // d_neighbors,
            //                                                                     d_neighborsCount, d_x, d_y, d_z, d_h, d_m, d_ro);
...
void computeDensity(const Task &task, ParticleData &d)
{
    ...
    kernels::density<<<blocksPerGrid, threadsPerBlock>>>(n, d.sincIndex, d.K, task.ngmax, d.d_bbox, d_clist, d_neighbors, d_neighborsCount, d.d_x,
                                                         d.d_y, d.d_z, d.d_h, d.d_m, d.d_ro);
    CHECK_CUDA_ERR(cudaGetLastError());

    // CHECK_CUDA_ERR(cudaMemcpyAsync(d.ro.data() + task.clist.front(), d_ro + task.clist.front(), task.clist.size() * sizeof(T), cudaMemcpyDeviceToHost));
...
void copyOutDensity(Dataset &d)
{
    ...
    // CHECK_CUDA_ERR(cudaMemcpyAsync(d.ro.data() + task.clist.front(), d.d_ro + task.clist.front(), task.clist.size() * sizeof(T),
        ...
        CHECK_CUDA_ERR(cudaMemcpyAsync(d.ro.data(), d.d_ro, size_np_T,
                                   cudaMemcpyDeviceToHost));
