...
void computeDensity(const std::vector<int> &l, Dataset &d)
{
    ...
    #pragma acc parallel loop copyin(n, clist [0:n], neighbors [0:allNeighbors], neighborsCount [0:n], \
                                     m [0:np], h [0:np], x [0:np], y [0:np], z [0:np]) copyout(ro [0:n])
    ...
    //#pragma omp parallel for
        ...
        //        // store the distance from each neighbor
