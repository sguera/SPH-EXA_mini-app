...
!=====================================================!
    ...
    SUBROUTINE output
      ...
      DOUBLE PRECISION,DIMENSION(nmax)::vrad,up,dw,gpr,fgr
      CHARACTER*17 nomfs
      CHARACTER*9 nomfs1
      CHARACTER*3 prefix1
      CHARACTER*6 sufix
      ...
      if(flags)write(*,*)'Escritura de datos'
...
!--------------------  User can change this  --------------------
         ...
         do i=1,n
            ...
            macum=macum+masa(k)
            if (outofNRup(k))up(k)=1.d0
            if (outofNRdw(k))dw(k)=1.d0
            write(10,formatout) k,(ps(j),j=1,dim),h(k),u(k),promro(k),&
                 ...
                 &    macum,ugrav(k),c(k),indice(k),checkInorm(k),up(k),dw(k),&
                 &    fgr(k),gpr(k)
         enddo
!----------------------------------------------------------------
